return {
  ...
  opts = function()
    local function natural_cmp(left, right)
      if left.type ~= "directory" and right.type == "directory" then return false end
      if left.type == "directory" and right.type ~= "directory" then return true end
      ...
      for i = 1, maxlen do
        ...
        if ln and rn then
          ...
          if lnum ~= rnum then return lnum < rnum end
    ...
    return {
      sort = { sorter = function(nodes) table.sort(nodes, natural_cmp) end },
      view = { width = 35 },
      renderer = {
